...
export default function ProgressTrackingScreen({ pattern, onNavigate }: ProgressTrackingScreenProps) {
  ...
  const achievements = [
    { icon: "🌸", label: "First Stitch", unlocked: done >= 1, color: "#C24E6B" },
    { icon: "🧶", label: "10 Steps Done", unlocked: done >= 10, color: "#7C5FA8" },
    { icon: "⭐", label: "Half Way", unlocked: pct >= 50, color: "#D4921A" },
    { icon: "🏆", label: "Pattern Done", unlocked: pct >= 100, color: "#84934F" },
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-6 pt-5 pb-4" style={{ borderBottom: "1px solid rgba(140,100,55,0.15)" }}>
        <button onClick={() => onNavigate("viewer")} className="w-8 h-8 rounded-full flex items-center justify-center hover:opacity-70" style={{ background: "rgba(194,78,107,0.08)", color: "#C24E6B" }}>
          <ChevronLeft className="h-4 w-4" />
        </button>
        <div>
          <h1 className="font-heading font-bold text-[22px]" style={{ color: "#3D2318" }}>Progress Tracking</h1>
          {pattern && <p className="text-[12px]" style={{ color: "#9A7868" }}>{pattern.title}</p>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 pb-20 md:pb-4 flex flex-col gap-4">
        ...
        {!pattern ? (
          <div className="craft-card p-6 flex flex-col items-center text-center gap-3">
            ...
            <p className="text-[12px]" style={{ color: "#9A7868" }}>Open a pattern from your library to track its progress.</p>
          ...
          <>
            ...
            <div className="craft-card p-5 flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
              <div className="relative flex-shrink-0" style={{ width: 100, height: 100 }}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="font-heading font-bold text-[22px] leading-none" style={{ color: "#C24E6B" }}>{pct}%</span>
                  <span className="text-[9px] font-semibold" style={{ color: "#9A7868" }}>done</span>
                </div>
              </div>
              <div className="flex flex-col gap-1.5 text-center sm:text-left">
                <div>
                  <p className="font-heading font-bold text-[28px] leading-none" style={{ color: "#3D2318" }}>{done}/{total}</p>
                  <p className="text-[12px]" style={{ color: "#9A7868" }}>steps complete</p>
                </div>
                {timeLabel && (
                  <div className="flex items-center justify-center sm:justify-start gap-1.5">
                    <Clock className="h-3.5 w-3.5" style={{ color: "#84934F" }} />
                    <span className="text-[11px] font-semibold" style={{ color: "#84934F" }}>{timeLabel}</span>
                  </div>
                )}
                {pattern.status && (
                  <div className="flex items-center justify-center sm:justify-start gap-1.5">
            ...
            <div className="craft-card p-4">
              <p className="font-heading font-semibold text-[13px] mb-3" style={{ color: "#3D2318" }}>By section</p>
              <div className="flex flex-col gap-3">
                {realSections.length === 0 && (
                  <p className="text-[12px]" style={{ color: "#9A7868" }}>This pattern has no sections yet.</p>
                )}
                  ...
                  return (
                    <div key={i}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[11px]" style={{ color: "#9A7868" }}>{sDone}/{s.steps.length}</span>
                      </div>
                      <div className="h-2.5 rounded-full overflow-hidden" style={{ background: "rgba(194,78,107,0.10)" }}>
                        <div className="h-full rounded-full transition-all" style={{ width: `${sPct}%`, background: sPct === 100 ? "#84934F" : "#C24E6B" }} />
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="craft-card p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="font-heading font-semibold text-[13px]" style={{ color: "#3D2318" }}>Achievements</p>
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => onNavigate("stitch-counter")} className="flex-1 py-2.5 rounded-xl text-[12px] font-bold transition-all hover:opacity-90"
                style={{ background: "rgba(132,147,79,0.12)", color: "#84934F", border: "1px dashed rgba(132,147,79,0.4)" }}>🧮 Stitch Counter</button>
            </div>
          </>
        )}
      </div>
